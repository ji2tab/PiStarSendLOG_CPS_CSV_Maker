...
本プラグインのH1用CSV出力では、カラム名にとらわれず「無線機の画面で見たときに役立つ情報」を優先して各列に配置しています。
...
> これはあくまでJJ2YYKクラブでの運用方針に基づく成形です。無線機や運用スタイルによって最適な配置は異なります。必要に応じて通常CSVをベースに独自に加工していただくことも可能です。
...
**ファイル名例:** `dmr_user_Japan_20260414123456.csv`
...
**ファイル名例:** `dmr_h1_Japan_20260414123456.csv`

---

## 認証仕様

- コールサインとDMR IDの両方が一致した場合のみダウンロード可能
...
| バージョン | 日付 | 変更内容 |
|---|---|---|
| **1.4.3** | 2026-04-14 | 入力欄のプレースホルダーテキストを削除。入力欄をdivレイアウトに変更（スマホ対応） |
| **1.4.2** | 2026-04-14 | 入力欄のプレースホルダーを汎用表現に変更 |
| **1.4.1** | 2026-04-10 | H1用CSVの列マッピングを実機フォーマットに完全一致させる修正 |
| **1.4.0** | 2026-04-10 | H1用CSV生成機能を追加。ダウンロードログ機能を追加 |
| **1.3.0** | 2026-04-08 | 国フィルター機能を追加 |
| **1.0.0** | 2026-04-08 | 初版リリース |
