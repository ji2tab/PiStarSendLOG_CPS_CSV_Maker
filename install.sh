...
if [ ! -f "${INSTALL_DIR}/${SCRIPT_NAME}" ]; then
    echo -e "${RED}エラー: スクリプトのダウンロードに失敗しました。${NC}"
    exit 1
fi
...
read -p "WordPressサーバーのドメイン名 (例: jj2yyk.forums.gr.jp): " input_host < /dev/tty
read -p "このホットスポットの識別ノード名 (例: yyk-tgif): " input_node < /dev/tty
read -p "APIトークン (WordPress側で設定したパスワード): " input_token < /dev/tty
